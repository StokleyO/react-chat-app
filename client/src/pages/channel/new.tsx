...
const createChannel = useMutation({mutationFn: ({name, memberIds, imageUrl}: {name: string, memberIds: string[], imageUrl?: string}) => {
    if (streamChat == null) throw Error("Not connected")

    return streamChat.channel("messaging", crypto.randomUUID(), {name, image: imageUrl, members: [user.id, ...memberIds]}).create()
},
onSuccess() {
    navigate("/")
},
...
function handleSubmit(e: FormEvent) {
    ...
    return (
        <FullScreenCard>
            ...
            <Select ref={memberIdsRef} id="members" required isMulti classNames={{container: () => "w-full"}} isLoading={users.isLoading} options={users.data?.users.map(user => { return {value: user.id, label: user.name || user.id}})} />

            <Button disabled={createChannel.isLoading} type="submit" className='col-span-full'>{createChannel.isLoading ? "Creating..." : "Create"}</Button>
            </form>
            </FullScreenCard.Body>
